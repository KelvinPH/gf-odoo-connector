...
define( 'GF_ODOO_VERSION', '1.6.0' );
define( 'GF_ODOO_PATH', plugin_dir_path( GF_ODOO_FILE ) );
define( 'GF_ODOO_URL', plugin_dir_url( GF_ODOO_FILE ) );

/**
 * Cache-busting version string for a plugin asset (version + filemtime).
 *
 * @param string $relative_path Path relative to the plugin root, e.g. "assets/js/admin.js".
 *
 * @return string
 */
function gf_odoo_asset_version( string $relative_path ): string {
	$path  = GF_ODOO_PATH . ltrim( $relative_path, '/' );
	$mtime = file_exists( $path ) ? (string) filemtime( $path ) : '0';

	return GF_ODOO_VERSION . '.' . $mtime;
}
...
function gf_odoo_connector_autoload( $class ) {
	$classes = array(
		...
		'Partner_Handler'        => 'class-partner-handler.php',
		...
		'GF_Odoo_Product_Tag_Map' => 'class-product-tag-map.php',
		'GF_Odoo_Ticket_Category_Map' => 'class-ticket-category-map.php',
		'GF_Odoo_Encryption'     => 'class-encryption.php',
		'GF_Odoo_GitHub_Updater' => 'class-github-updater.php',
		'GF_Odoo_Lead_Classifier' => 'class-lead-classifier.php',
		'GF_Odoo_AI_Classifier'   => 'class-ai-classifier.php',
	);
